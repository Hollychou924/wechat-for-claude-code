...
- 在微信中直接跟 Claude 对话，获得和 Claude Code 一样的完整能力
- 每个微信用户独立会话，上下文自动保持（可跨多条消息连续对话）
- **发图片/文件给 Claude**——微信发图或发文件，Claude 自动识别内容并回复
- **Claude 生成的图片自动回传微信**——不再只是文字，图片也能直接看到
- **远程图片 URL 自动发送**——Claude 回复中包含图片链接，自动下载并以图片消息发送
- 语音消息自动转文字，发语音也能用
- 引用回复带上下文，Claude 能看到你引用的内容
- 发送"新对话"随时重置上下文，开始全新话题
- 发送"调试"查看运行状态、会话信息、内存占用
...
| **微信端命令** | `新对话` 重置 / `调试` 状态 / `帮助` 查看命令 | 无 |
| **图片/文件支持** | 接收图片文件 Claude 自动识别；Claude 生成图片自动回传微信 | 静默忽略 |
...
├── api.ts         # WeChat ilink API 客户端（含图片发送）
├── auth.ts        # QR 扫码登录（支持自动刷新）
├── cdn.ts         # CDN 媒体下载/上传 + AES-128-ECB 加解密
├── message.ts     # 消息解析 + 媒体提取 + markdown 转纯文本
├── claude.ts      # Claude Code CLI 调用 + 会话管理
├── polling.ts     # 长轮询 + per-user 消息队列 + 图片回传
...
- 图片/文件接收：CDN 下载 + AES-128-ECB 解密，Claude 自动识别内容
- 图片回传微信：Claude 生成的图片自动上传 CDN 并以图片消息发送
- 远程图片自动发送：检测回复中的图片 URL，自动下载并发送到微信
- 调试命令：微信发"调试"查看运行状态、会话、内存等信息
